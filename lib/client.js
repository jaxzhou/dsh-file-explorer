...
__export(index_exports, {
  ...
  MAX_DOCUMENT_IMAGES: () => MAX_DOCUMENT_IMAGES,
  ...
  relativeImageDestinations: () => relativeImageDestinations,
...
// src/client/markdown-assets.ts
var MAX_DOCUMENT_IMAGES = 24;
var MAX_ASSET_BYTES = 8 * 1024 * 1024;
var IMAGE_REFERENCE = /!\[[^\]]*\]\(\s*(?:<([^<>\n]*)>|([^\s()]*(?:\([^\s()]*\)[^\s()]*)*))(?:\s+(?:"[^"]*"|'[^']*'|\([^()]*\)))?\s*\)/g;
var FENCED_BLOCK = /^[ \t]*(?:```|~~~)[\s\S]*?^[ \t]*(?:```|~~~)[^\n]*$/gm;
var INLINE_CODE = /`[^`\n]*`/g;
...
function relativeImageDestinations(text) {
...
function resolveRelativePath(baseDirectory, destination) {
...
var ASSET_CACHE_LIMIT = 64;
function cacheAsset(path, dataUrl) {
...
function RenderedMarkdown({
  ...
  const destinations = (0, import_react.useMemo)(() => relativeImageDestinations(text), [text]);
  ...
  const pathImages = (0, import_react.useMemo)(
    () => ({ resolve: (value) => assets[value] }),
    [assets]
  );
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "dsh-fe-prose", ref: proseRef, "data-preview-format": "markdown", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_dsh_client_ui_primitives.MarkdownText, { text, labels, pathImages }) });
}
...
function filesFace(remote, sessionId, signal, actions) {
  ...
  return {
    ...
    readImage(path, request) {
      if (signal.aborted || request.aborted) return Promise.resolve(void 0);
      const format = previewFormatFor(path);
      if (format.kind !== "image") return Promise.resolve(void 0);
      return remote.workspaceFiles.readAll(sessionId, path, request).then((result) => {
        if (!result.ok || request.aborted) return void 0;
        const { bytes, data } = result.value;
        if (bytes !== void 0 && bytes > MAX_ASSET_BYTES) return void 0;
